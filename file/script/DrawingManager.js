    ...
    DrawingManager.prototype._calculate = function (overlay, point) {
        ...
        if (this._enableCalculate && BMapLib.GeoUtils) {
            ...
            if (!result.data || result.data < 0) {
                ...
                if(type == "[object Polyline]"){
                    //保留2位小数位
                    result.data = "长度:"+(result.data/1000).toFixed(1)+"公里";
                    result.label = this._addLabel(point, result.data);
                }else if(type== "[object Polygon]"||type=="[object Circle]"){
                    result.data = "面积:"+result.data.toFixed(0)+"平方米";
                    result.label = this._addLabel(point, result.data);
                }
